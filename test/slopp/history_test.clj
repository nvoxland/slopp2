...
(deftest form-at-delta-travels-through-a-forms-versions
  (let [sess (api/open!)]
    (try
...
(deftest form-at-delta-edge-cases
  (let [sess (api/open!)]
    (try
      (api/ingest! sess 'hi.core seed)
...
(deftest form-at-delta-follows-renames
  (let [sess (api/open!)]
    (try
      (api/ingest! sess 'hi.core seed)
...
(deftest was-green-at-reads-the-verification-arc
  (let [sess (api/open!)]
    (try
      (api/ingest! sess 'hi.core seed)
...
(deftest form-history-versions-carry-was-green-at
  (let [sess (api/open!)]
    (try
      (api/ingest! sess 'hi.core seed)
...
(deftest form-at-delta-rides-the-mcp-surface
  (let [sess (api/open!)]
    (try
      (api/ingest! sess 'hi.core seed)
