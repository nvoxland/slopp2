...
(deftest record-commit-extra-round-trips
...
(deftest commit-point-snapshots-the-rendered-tree
  (let [sess (api/open!)]
    (try
...
(deftest projection-mints-deterministic-shas
  (let [dir  (temp-dir)
...
(deftest branch-shares-prefix-shas
  (let [dir  (temp-dir)
        sess (api/open! {:dir dir})]
    (try
      (api/ingest! sess 'gp.core seed)
...
(deftest retroactive-target-is-lossy-but-pinned
  (let [dir  (temp-dir)
        sess (api/open! {:dir dir})]
    (try
      (api/ingest! sess 'gp.core seed)
...
(deftest forced-red-milestone-carries-status-trailer
  (let [dir  (temp-dir)
        sess (api/open! {:dir dir})]
    (try
      (api/ingest! sess 'gp.core seed)
